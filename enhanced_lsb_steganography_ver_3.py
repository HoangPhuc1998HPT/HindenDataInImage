...
from PyQt5.QtWidgets import (QVBoxLayout, QHBoxLayout, QSplitter, QFileDialog,
...
class SteganographyGUI(QtWidgets.QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.security_results.setMaximumHeight(200)
    ...
    def setup_comparison_table(self):
        ...
        headers = ["Method", "PSNR (dB)", "MSE", "Embed Time", "Extract Time", "Security Score"]
        ...
        for row, (method, display_name) in enumerate(zip(methods, display_names)):
            if method in data:
                ...
                for col, item_text in enumerate(items):
                    ...
                    if col > 0:  # Skip method name
                        if method == 'enhanced':
                            # Highlight if enhanced is better
                            if ((col == 1 and d['psnr'] > data.get('traditional', {}).get('psnr', 0)) or
                                    (col == 5 and d['security_score'] > data.get('traditional', {}).get(
                                        'security_score', 0))):
    ...
    def clear_results(self):
        ...
        for canvas in [self.image_canvas, self.histogram_canvas,
