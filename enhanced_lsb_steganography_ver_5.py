...
class SteganographyGUI(QtWidgets.QMainWindow):
    ...
    def setup_ui(self):
        ...
        batch_group = QtWidgets.QGroupBox("4. Export & Advanced Operations")
        batch_layout = QtWidgets.QHBoxLayout(batch_group)

        # Export images buttons
        export_images_btn = QtWidgets.QPushButton("💾 Export Stego Images")
        export_images_btn.clicked.connect(self.export_stego_images)

        export_diff_btn = QtWidgets.QPushButton("🖼️ Export Difference Maps")
        export_diff_btn.clicked.connect(self.export_difference_maps)

        # Other buttons
        batch_btn = QtWidgets.QPushButton("📁 Batch Process Directory")
        batch_btn.clicked.connect(self.batch_process)

        export_btn = QtWidgets.QPushButton("📄 Export Text Results")
        export_btn.clicked.connect(self.export_results)

        clear_btn = QtWidgets.QPushButton("🗑️ Clear Results")
        clear_btn.clicked.connect(self.clear_results)

        batch_layout.addWidget(export_images_btn)
        batch_layout.addWidget(export_diff_btn)
    ...
    def export_stego_images(self):
        """Export stego images to files"""
        if not self.stego_images:
            QtWidgets.QMessageBox.warning(self, "Warning", "No stego images to export")
            return

        # Choose directory
        directory = QFileDialog.getExistingDirectory(
            self, "Select Directory to Save Stego Images"
        )
        ...
        try:
            ...
            for method, stego_array in self.stego_images.items():
                ...
                if len(stego_array.shape) == 3:
                    # RGB image
                    stego_pil = Image.fromarray(stego_array.astype(np.uint8))
                else:
                    # Grayscale image
                    stego_pil = Image.fromarray(stego_array.astype(np.uint8), mode='L')

                # Generate filename
                timestamp = time.strftime("%Y%m%d_%H%M%S")
                filename = f"stego_{method}_{timestamp}.png"
                filepath = os.path.join(directory, filename)

                # Save image
                stego_pil.save(filepath, "PNG")
                exported_files.append(filepath)

                # Also save with message info in filename
                safe_message = "".join(
                    c for c in self.current_message[:20] if c.isalnum() or c in (' ', '-', '_')).rstrip()
                if safe_message:
                    filename_with_msg = f"stego_{method}_{safe_message}_{timestamp}.png"
                    filepath_with_msg = os.path.join(directory, filename_with_msg)
                    stego_pil.save(filepath_with_msg, "PNG")
                    exported_files.append(filepath_with_msg)

            # Show success message
            files_list = "\n".join([os.path.basename(f) for f in exported_files])
            QtWidgets.QMessageBox.information(
                self, "Export Successful",
                f"Exported {len(exported_files)} stego images:\n\n{files_list}"
            )

        except Exception as e:
            QtWidgets.QMessageBox.critical(self, "Export Error", f"Error exporting stego images: {str(e)}")

    def export_difference_maps(self):
        """Export difference maps as images"""
        if not self.stego_images:
            QtWidgets.QMessageBox.warning(self, "Warning", "No stego images to generate difference maps")
            return

        # Choose directory
        directory = QFileDialog.getExistingDirectory(
            self, "Select Directory to Save Difference Maps"
        )
        ...
        try:
            ...
            for method, stego_array in self.stego_images.items():
                ...
                # Save original difference (subtle)
                diff_filename = f"diff_original_{method}_{timestamp}.png"
                diff_filepath = os.path.join(directory, diff_filename)
                cv2.imwrite(diff_filepath, diff)
                exported_files.append(diff_filepath)

                # Save amplified difference (for visualization)
                diff_amplified = np.clip(diff.astype(float) * 50, 0, 255).astype(np.uint8)
                diff_amp_filename = f"diff_amplified_{method}_{timestamp}.png"
                diff_amp_filepath = os.path.join(directory, diff_amp_filename)
                cv2.imwrite(diff_amp_filepath, diff_amplified)
                exported_files.append(diff_amp_filepath)

                # Create colored difference map using matplotlib
                plt.figure(figsize=(10, 8))
                plt.imshow(diff_amplified, cmap='hot', vmin=0, vmax=255)
                plt.colorbar(label='Difference (amplified)')
                plt.title(f'Difference Map - {method.title()} LSB')
                plt.axis('off')

                colored_filename = f"diff_colored_{method}_{timestamp}.png"
                colored_filepath = os.path.join(directory, colored_filename)
                plt.savefig(colored_filepath, dpi=300, bbox_inches='tight')
                plt.close()
                exported_files.append(colored_filepath)

            # Show success message
            files_list = "\n".join([os.path.basename(f) for f in exported_files])
            QtWidgets.QMessageBox.information(
                self, "Export Successful",
                f"Exported {len(exported_files)} difference map files:\n\n{files_list}"
            )

        except Exception as e:
            QtWidgets.QMessageBox.critical(self, "Export Error", f"Error exporting difference maps: {str(e)}")

    def export_all_results(self):
        """Export everything: images, difference maps, and text results"""
        if not self.stego_images:
        ...
        # Choose directory
        directory = QFileDialog.getExistingDirectory(
            self, "Select Directory for Complete Export"
        )
        ...
        try:
            ...
            timestamp = time.strftime("%Y%m%d_%H%M%S")
            export_dir = os.path.join(directory, f"steganography_export_{timestamp}")
            os.makedirs(export_dir, exist_ok=True)

            images_dir = os.path.join(export_dir, "stego_images")
            diff_dir = os.path.join(export_dir, "difference_maps")
            os.makedirs(images_dir, exist_ok=True)
            os.makedirs(diff_dir, exist_ok=True)

            # Export original directory temporarily change
            original_dir = os.getcwd()

            # Export stego images
            temp_dir = images_dir
            self.export_stego_images_to_directory(temp_dir)

            # Export difference maps
            temp_dir = diff_dir
            self.export_difference_maps_to_directory(temp_dir)

            # Export text results
            results_file = os.path.join(export_dir, f"analysis_results_{timestamp}.txt")
            with open(results_file, 'w') as f:
                f.write("=== COMPLETE STEGANOGRAPHY ANALYSIS RESULTS ===\n\n")
                f.write(f"Export Date: {time.strftime('%Y-%m-%d %H:%M:%S')}\n")
                f.write(f"Original Message: {self.current_message}\n")
                f.write(f"Secret Key: {self.current_key}\n\n")
            ...
            QtWidgets.QMessageBox.information(
                self, "Complete Export Successful",
                f"All results exported to:\n{export_dir}\n\n"
                f"Contents:\n"
                f"- Stego images in /stego_images/\n"
                f"- Difference maps in /difference_maps/\n"
                f"- Analysis results in analysis_results_{timestamp}.txt"
            )

        except Exception as e:
            QtWidgets.QMessageBox.critical(self, "Export Error", f"Error during complete export: {str(e)}")

    def export_stego_images_to_directory(self, directory):
        """Helper method to export stego images to specific directory"""
        exported_files = []
        timestamp = time.strftime("%Y%m%d_%H%M%S")
    ...
    def export_difference_maps_to_directory(self, directory):
        """Helper method to export difference maps to specific directory"""
        # Convert cover image to grayscale if needed
        if len(self.cover_image.shape) == 3:
            cover_gray = cv2.cvtColor(self.cover_image, cv2.COLOR_RGB2GRAY)
        else:
        ...
        for method, stego_array in self.stego_images.items():
            ...
            # Calculate and save difference maps
            diff = cv2.absdiff(cover_gray, stego_gray)
            diff_amplified = np.clip(diff.astype(float) * 50, 0, 255).astype(np.uint8)

            # Save amplified difference
            diff_filename = f"diff_{method}_{timestamp}.png"
            diff_filepath = os.path.join(directory, diff_filename)
            cv2.imwrite(diff_filepath, diff_amplified)
            exported_files.append(diff_filepath)
